...
I think they were asking the wrong question.   If you claim that "la Quinta" means "close to Denny's", you should ask ... for each la Quinta hotel, is there a Denny's nearby. In the analysis above, we computed, "for each Denny's ...."

We define a new state analysis function to swap that question, simply achieved by swapping the order of the join.
...
analyze_state_q = function (this_state) {
  ...
  dn_lq_state_mindist <- full_join(lq_state, dn_state, by = "state") %>%
    mutate(distance=
           haversine(longitude.x, latitude.x, longitude.y, latitude.y, round = 4)) %>%
  group_by(address.x) %>%
  summarise(closest = min(distance))  
...
On Alaska, are result now has two rows:

```{r}
dfakq=analyze_state_q("AK")
```

And Texas:

```{r}
dftxq=analyze_state_q("TX")
nrow(dftxq)
```

which actually has more La Quinta's than Denny's.  Let's visualize.

```{r}
dftxq %>% ggplot(aes(x=closest)) + geom_histogram()
```

There is not much difference to the story, but, perhaps a little.

### Comparing close - Denny's vs other restaurants

As discussed in class, we would want to more clearly define "close."   

"Close," compared to what?

I would assert that maybe we mean "closer than typical restaruants".

#### More data

To compare with other restaurants, we need more data.  I found a data set on Kaggle (https://www.kaggle.com/datafiniti/fast-food-restaurants?select=FastFoodRestaurants.csv) that provides a sampling of restaurants and locations (though not a complete list).


```{r}
dfall=read_csv("FastFoodRestaurants.csv")
```

10000 data points.  

Note that Denny's is not on the list, as these are only fast food restaurants. 

```{r}
dfall %>% filter(name=="Dennys")
```


Regardless, of this difference, I believe we can do a reasonably fair test.

#### A fair comparison

Consider just Texas.  If we take a random sample (from the restaurant dataset) of the same size as we have for the number of Denny's in Texas, and repeat our analysis (of how close is the nearest restaurant), I think we can make a fair comparison (so long as the original dataset includes Texas.)

Let's do a quick check.

```{r}
dfall %>% filter(province=="TX") 
```
Plenty of data, so pulling a sample of size 200 will easy.

We build a new function that appropriately samples a comparatively sized sample:

```{r}
analyze_state_r = function (this_state) {
  
  s=dn_state <- dn %>%
  filter(state == this_state) %>% nrow()  # samples required
  
  rest_state=dfall %>% filter(province==this_state) %>% 
    rename(state=province) %>% sample_n(s)
  
  lq_state = lq %>%
  filter(state == this_state)
  
  dn_lq_state_mindist <- full_join(lq_state, rest_state, by = "state") %>%
    mutate(distance=
           haversine(longitude.x, latitude.x, longitude.y, latitude.y, round = 4)) %>%
  group_by(address.x) %>%
  summarise(closest = min(distance))  
...
Now let's compute on Texas and visualize:

```{r}
dftx_rest = analyze_state_r("TX")

dftx_rest %>% ggplot(aes(x=closest)) + geom_histogram()
summary(dftx_rest)
summary(dftxq)
```
...
```{r}
dfny_rest = analyze_state_r("NY")
summary(dfny_rest)
dfny_rest %>% ggplot(aes(x=closest)) + geom_histogram()
```

As compared with Denny's:

```{r}
analyze_state_q("NY") %>%  summary()
```
